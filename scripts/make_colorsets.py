...
def main() -> None:
    ...
    # THIS SCRIPT DOES NOT TOUCH AppIcon.appiconset.
    #
    # It used to write a placeholder Contents.json there. Once make_icon.py
    # produced the real icon, the two scripts owned the same file with different
    # contents, and CI's "asset catalog is out of date" drift check - which works
    # by re-running this script and diffing - failed on every push. The check was
    # right; the ownership was wrong.
    #
    # Colour sets are generated here. The icon is generated by make_icon.py and
    # validated by check_app_icon.py. One owner per file.
    print(f"\n{len(PALETTE)} colour sets generated in {CATALOG.relative_to(ROOT)}")
    print("AppIcon.appiconset is owned by scripts/make_icon.py — not touched here.")
